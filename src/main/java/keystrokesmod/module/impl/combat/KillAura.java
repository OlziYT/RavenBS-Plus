...
public class KillAura extends Module {
    ...
    public void guiUpdate() {
        this.showTargetNumber.setVisible(infoMode.getInput() == 3, this);
    }

    @Override
    public String getInfo() {
        switch ((int) infoMode.getInput()) {
            case 0:
                return autoBlockModes[(int) autoBlockMode.getInput()];
            case 1:
                return rotationModes[(int) rotationMode.getInput()];
            case 2:
                return String.format("%.1f APS", aps.getInput());
            case 3:
                int targetCount = getTargetCount();
                if (targets.getInput() == 1 || targets.getInput() == 2) {
                    return "Single";
                } else if (targets.getInput() >= 3 && targets.getInput() <= 5) {
                    return "Switch" + (showTargetNumber.isToggled() ? " " + targetCount : "");
                } else {
                    return "Multi" + (showTargetNumber.isToggled() ? " " + targetCount : "");
                }
            default:
                return "";
        }
    ...
    private int getTargetCount() {
        ...
        return availableTargets.isEmpty() ? 0 : availableTargets.size();
    }
    ...
    public boolean blinkAutoBlock() {
        return (autoBlockMode.getInput() == 4 || autoBlockMode.getInput() == 5);
    }
